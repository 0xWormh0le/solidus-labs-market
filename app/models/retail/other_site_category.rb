...
module Retail
  class OtherSiteCategory < RetailScraperRecord
    ...
    belongs_to :parent, class_name: 'Retail::OtherSiteCategory', foreign_key: 'parent_category_id', optional: true

    has_many :children, class_name: 'Retail::OtherSiteCategory', foreign_key: 'parent_category_id'
    has_many :siblings, -> { where(parent_category_id: parent_category_id) }, class_name: 'Retail::OtherSiteCategory'

    scope :mapped, ->{ where('category_id IS NOT NULL') }
