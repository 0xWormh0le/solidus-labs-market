	...
	if (data && data != "") {
	  ...
	  $(".secondary-option-type-row .option-value-button").each(function(idx){
	    ...
	    if (commonVariantIds.length < 1) {
	      $(this).removeClass('btn-available').addClass('btn-unavailable').removeClass('selected');
	    } else {
...
$(document).ready(function(){
	...
	$('#option_type_row_1 .option-value-btn').click(selectOptionValue);
	$('.secondary-option-type-row .option-value-btn').click(selectSecondaryOptionValue);
})
